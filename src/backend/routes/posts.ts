...
export const postsRoutes = new Elysia({ prefix: "/api/posts" })
  ...
  .post("/actionPosts/addLike", async ({ body, set }: any) => {
    try {
      ...
      if (!isValidObjectId(postId) || !userId) {
        set.status = 400;
        return { success: false, message: "Post ID and User ID are required." };
      }

      const postSnapshot = (await Posts.findById(postId)
        .select("likedBy")
        .lean()) as IPost | null;
      ...
      // Use toString() to ensure comparison works if one is an ObjectId
      const userAlreadyLiked = postSnapshot.likedBy.some(
      ...
      return {
        success: true,
        message: message,
        likedStatus: isNowLiked, // Changed 'liked' to 'likedStatus' to match your frontend logic
